...
```
Cette commande va lancer un entraînement sur le dataset "samples_100", avec un model de 1.6M paramètres, pendant au max 10 époques ou 20 minutes (une époque commencée n'est pas interrompue).
...
Si le tokenizer ciblé n'existe pas déjà, il sera entraîné puis sauvegardé automatiquement.
Les noms de sauvegardes (save_name et tokenizer_name) peuvent être paramétrés comme on veut.
Les chemins exactes où sont sauvegardés les différentes choses seront indiqués à chaque fois dans la console.
L'entraînement a impérativement besoin du GPU pour fonctionner (n'a pas pu être testé sur Mac)
...
La génération est beaucoup plus rapide sur GPU mais elle peut aussi fonctionner sur CPU s'il n'est pas disponible.
